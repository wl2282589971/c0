...
namespace miniplc0 {
	...
	public:
		Analyser(std::vector<Token> v)
			...
			_uninitialized_vars({}), _vars({}), _consts({}), _nextTokenIndex(0) ,_cur_dimension(0){}
	...
	private:
        ...
        //定义类型
        std::optional<CompilationError> initDeclaration(std::string);
        //初始化声明
        std::optional<CompilationError> initDeclarationList(std::string);
        //多个变量初始化声明
        std::optional<CompilationError> initializer();
        //初始化
        std::optional<CompilationError> parameterClause();
        //参数
        std::optional<CompilationError> compoundStatement();
        //函数主过程
        std::optional<CompilationError> parameterDeclarationList();
        //参数列表
        std::optional<CompilationError> functionCall();
        //函数调用
        std::optional<CompilationError> expressionList();
        //表达式列表
        std::optional<CompilationError> statementSeq();
        //语句序列
        ...
        //输出语句
        std::optional<CompilationError> printlist();
        //输出列表
        std::optional<CompilationError> printable();
        //输出内容
        std::optional<CompilationError> scanStatement();
        //输入语句
        std::optional<CompilationError> assigmentExpression();
        //执行表达式
	...
	private:
        ...
        std::map<std::string, int32_t> _functions;
        //符号表构建
        ...
        std::map<std::string,std::vector<property>>;
